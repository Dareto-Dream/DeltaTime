class Api::Deltatime::V1::DeltatimeController < ApplicationController
  ...
  def push_heartbeats
    if params["format"] == "bulk"
      # POST /api/deltatime/v1/users/:id/heartbeats.bulk
      ...
      # POST /api/deltatime/v1/users/:id/heartbeats
  ...
  def heartbeat_bulk_params
    if params[:_json].present?
      ...
      deltatime = params.require(:deltatime)
      { heartbeats: permit_heartbeat_collection(deltatime[:heartbeats]) }
    end
  end
  ...
  def heartbeat_params
    ...
    elsif params[:deltatime].present?
      permit_heartbeat(params.require(:deltatime)) || {}
    else
